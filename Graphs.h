...
// Function Prototypes___________________
	...
	void shPath(int s);
	void printShortestPath(int , int) const;			// Shortest paths from node (s)
	void showDFS();
	void print_into_file(string);
//______________________________________________________________________________________________
   ...
   private:
	   ...
	   void printPath(int, int) const;	// Print path from source (s) to destination (i)
	   void visit(int k);	
	   int minDistance(vector<pair<int,string>>, bool[]);
	   void printSolution(vector<pair<int,string>> ,int );
};
#endif // GRAPHS_H